  ...
  if (state.status === 'error') {
    return (
      <div className={cn('flex h-full flex-col items-center justify-center gap-2 px-6 text-center text-sm text-destructive', className)}>
        <p>{t('filePreview.sheet.loadFailed', { defaultValue: 'Failed to load spreadsheet: {{error}}', error: state.message })}</p>
      </div>
    );
  }
  ...
  if (!activeSheet) {
    return (
      <div className={cn('flex h-full items-center justify-center text-sm text-muted-foreground', className)}>
        {t('filePreview.sheet.empty', { defaultValue: 'This file has no displayable worksheets' })}
      </div>
    );
  }
      ...
      <div ref={scrollerRef} className="min-h-0 flex-1 overflow-auto">
        {isEmpty ? (
          <div className="flex h-full items-center justify-center px-6 text-center text-sm text-muted-foreground">
            {t('filePreview.sheet.emptySheet', { defaultValue: 'This worksheet is empty' })}
          </div>
        ) : (
        ...
        {totalPages > 1 && (
          <div className="flex shrink-0 items-center gap-1 border-l border-black/10 px-2 text-xs text-muted-foreground dark:border-white/10">
            <Button
              ...
              onClick={handlePrev}
              disabled={page <= 0}
              title={t('filePreview.sheet.prevPage', 'Previous rows')}
            >
            ...
            <Button
              ...
              title={t('filePreview.sheet.nextPage', 'Next rows')}
            >
