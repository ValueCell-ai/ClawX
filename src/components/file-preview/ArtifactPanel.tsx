 ...
 *     the graph, or “View changes” picks the latest change).
 ...
 *     context.
 *   - Workspace: read-only workspace tree + file preview,
 ...
 * toolbar buttons, "View changes →" links) can drive it.
 */
...
export interface ArtifactPanelProps {
  ...
  /** Used to mark "Updated this run" badges on the workspace tree. */
 ...
 * the run’s file cards or “View changes →” (auto-first); switching workspace
...
function ChangesTab({ files, focusedFile, onFocus }: ChangesTabProps) {
  ...
  if (!focusedFile && uniqueFiles.length === 0) {
    return (
      <div className="flex h-full items-center justify-center px-6 text-center text-sm text-muted-foreground">
        {t('artifactPanel.changes.empty', 'No file changes in this conversation yet')}
      </div>
    );
  }
  ...
  return focusedFile ? (
    ...
    <div className="flex h-full items-center justify-center px-6 text-center text-sm text-muted-foreground">
      {t('artifactPanel.changes.selectFileHint', 'Click a file card in the chat or “View changes”')}
    </div>
  );
}
      ...
      <div className="flex h-full flex-col items-center justify-center gap-2 px-6 text-center">
        ...
        <p className="max-w-md text-xs text-muted-foreground">
          {t(
            ...
            'Click a file card in the chat first to open the sidebar and select a file.',
          )}
        </p>
      </div>
    );
