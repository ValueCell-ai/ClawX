...
export function FilePreviewBody({
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const localized =
        code === 'outsideSandbox'
          ? t('filePreview.errors.outsideSandbox', 'Path outside sandbox — write denied')
          : code === 'readOnlyRoot'
            ? t('filePreview.errors.readOnlyRoot', 'File lives in a read-only location (for example a bundled skill) and cannot be modified')
  ...
  const renderBody = () => {
          ...
          <p>
            {directOpen
              ...
              : t('filePreview.errors.tooLarge', {
                defaultValue: 'File too large ({{size}}); preview disabled',
    ...
    if (state.status === 'binary') {
      return (
        <div className="flex h-full flex-col items-center justify-center gap-3 px-6 text-center text-sm text-muted-foreground">
          <p>{t('filePreview.errors.binary', 'Binary file — text preview unavailable')}</p>
          <Button variant="outline" size="sm" onClick={handleOpenInFinder}>
            <FolderOpen className="mr-2 h-4 w-4" />
            {t('filePreview.actions.openInFinder', 'Reveal in file manager')}
          </Button>
    ...
    if (state.status === 'outsideSandbox') {
      return (
        <div className="flex h-full flex-col items-center justify-center gap-4 px-8 text-center">
          ...
          <div className="space-y-1.5">
            <p className="text-sm font-medium text-foreground">
              {t('filePreview.errors.outsideSandboxTitle', 'Could not read this file')}
            </p>
            <p className="max-w-md text-xs leading-relaxed text-muted-foreground">
              {t(
                'filePreview.errors.outsideSandboxHint',
                'ClawX could not read this path. The file may have been moved or deleted, or the current account may not have permission. You can still reveal it in the file manager.',
              )}
            </p>
          </div>
          <Button variant="outline" size="sm" onClick={handleOpenInFinder}>
                      ...
                      <div className="flex h-full flex-col items-center justify-center gap-2 px-6 text-center">
                        <p className="max-w-md text-sm text-muted-foreground">
                          {t(
                            ...
                            'The current chat did not capture a precise baseline for this file, so no diff is available.',
                          )}
                        </p>
                        <p className="max-w-md text-2xs text-muted-foreground/90">
                          {t(
                            'filePreview.diff.unavailableHint',
                            'Use the Preview tab for the current file, or use Git or another VCS for an exact diff.',
                          )}
                        </p>
                      </div>
                    );
