...
expectedUserBehavior:
  ...
  - A config commit whose acknowledgement is lost to native reload is verified from the durable config instead of being reported as a false save failure; the comparison tolerates OpenClaw's auto-managed meta stamp and restored redaction sentinels, and an unverifiable loss replays the mutator through the file path.
