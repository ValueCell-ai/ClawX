...
The exported OpenClaw configuration must preserve diagnostic structure while recursively replacing values whose keys identify credentials, API keys, tokens, passwords, cookies, authorization values, or private keys. Exported logs must redact common bearer tokens and credential assignments. Before export, the UI must list the bundle categories and allow individual or select-all conversation selection.
