...
- Use a fresh public CUA session label per MCP transport, including reconnection within the same daemon generation. CUA rejects another transport's ended lease. Bump the bundled plugin package version when changing shipped plugin code so version-aware mirrors receive the fix.
- Screenshot results are model-only media and must not be forwarded to messaging channels.
- Normalize Command aliases to the pinned CUA driver's supported `cmd` modifier on macOS and Windows, not `meta`. Validate shortcut effects separately from input acknowledgments. Model sync may infer missing input modalities but must preserve explicit text-only declarations.
