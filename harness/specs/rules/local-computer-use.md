...
- Pin SDK, native driver, and official Skill together to 0.25.0 under `harness/specs/tasks/cua-025-upgrade.md`. Disable CUA product telemetry with `CUA_DRIVER_RS_TELEMETRY_ENABLED=false` in both Main's embedded daemon options and the macOS/Windows Gateway child environment. The native SDK allowlist supports telemetry variables since 0.22.0; the 0.21.0 rejection and CLI-only workaround in `harness/specs/tasks/cua-cli-telemetry.md` are superseded history, not current requirements. Validate manager-produced startup options with the real native constructor on supported hosts, without starting a daemon or requesting grants. Preserve parent/system environment and standalone settings; this is not an OS permission, model-controlled policy, or ClawX telemetry preference.
...
- The daemon executable must be version-pinned, checksum-verified before extraction, shipped outside ASAR, executable on POSIX systems, and signed before the enclosing macOS app is signed and notarized. Retain the Windows console-to-GUI PE subsystem patch after archive verification. Pinned 0.25.0 source still uses `cmd /c ver` for telemetry; the allowlist change is not an automatic upstream no-window fix. Report rebuilt Windows no-flash/stdio/cancellation and macOS package checks separately from historical evidence and synthetic tests.
...
- Use a unique explicit non-default workflow session across accepting one-shot CLI calls and `end_session` at cleanup. Pinned 0.25.0 `run_call` uses `cli-explicit` for named calls and disposable sessions for anonymous calls; document the discrepancy with upstream's broad disposable-CLI guidance rather than adding MCP. Invalidate observations and element tokens after generation changes. Verify live continuity separately from source/mocked contracts; do not retain obsolete per-MCP-transport lease requirements or claim unverified browser lifecycle/cleanup behavior.
...
- Retain the `computer-use` Skill name and `/computer-use` picker command. Vendor the official 0.25.0 accompanying Skill from tag `cua-driver-rs-v0.25.0`, commit `45d78fedcf2c7033ba33f10dd30f8af8ba31ec3f`, with MIT license, attribution, exact source file mapping/hashes, and a concise ClawX-specific entrypoint; never substitute moving upstream main. Upgrade untouched known 0.21.0 bundled files without leaving old version requirements active, preserving user modifications, unknown same-name Skills, and unrelated content. Do not relabel modified references as pristine upstream bytes or introduce a general updater. Selection never enables the daemon or grants permissions. See `harness/reference/computer-use-skill.md` for provenance and precedence.
