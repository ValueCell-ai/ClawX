...
HTTP(S) content links open in the system browser on ordinary activation and expose localized `Open in ClawX` and `Open in system browser` context-menu actions in all four locales. Main Renderer links use the project Radix context menu and restore focus; guest links use a localized native menu without a guest preload or bridge. Local `.html` and `.htm` file cards open in ClawX Web Browser by default.
