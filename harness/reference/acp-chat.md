...
A Gateway run may settle as aborted while its terminal Chat payload still carries buffered assistant text that was durably persisted before cancellation. The ACP bridge records any unseen suffix from that assistant message before settling the prompt as cancelled. Error-terminal messages remain errors rather than ordinary assistant prose.
