...
There are exactly three approved transcript-derived content supplements. ClawX may recover asynchronous image-generation completions with proven `image_generate` context, it may recover explicit line-leading assistant `MEDIA:` attachment directives omitted by OpenClaw ACP, and it may reconcile already-streamed assistant text for a settled live turn (see *Assistant Text Reconciliation* below). All three are bounded, marked, memory-only projections. Separately, Main may extract metadata-only whole-turn timing because ACP replay omits original timestamps. Renderer can attach that timing only to an unambiguously matched ACP turn; it cannot reconstruct thoughts, tool cards, plans, permissions, file activity, or missing turns. See `harness/reference/acp-generated-media-and-diagnostics.md#bounded-transcript-exceptions` for the content compatibility grammar and timing boundary.

### Assistant Text Reconciliation

**Rationale.** A correct ACP bridge is not sufficient to guarantee a complete reply. The delta path between the pinned bridge and the timeline is lossy in ways the bridge cannot observe: the Gateway broadcasts chat deltas with `dropIfSlow`, so a payload may be discarded under back-pressure, and both Main and Renderer drop updates whose session or generation no longer matches. Because `agent_message_chunk` carries no sequence number, offset, or length, a single lost chunk is undetectable in band and leaves a permanently short reply that only a conversation reload repairs. `fix-acp-multi-message-output-truncation` hardened the bridge and explicitly declined a transcript fallback; field evidence of a reply that rendered short while the persisted transcript held the full text superseded that decision.

**Source and scope.** The reconciliation reuses the existing bounded transcript read (`hostApi.sessions.history`, newest 1000 records) inside the live transcript-supplement operation, which runs immediately after a prompt settles and retries once after 1500 ms. It never runs for historical replay, which already comes from the transcript.

**Limits.** It pairs the turn's transcript assistant texts positionally with the same turn's streamed assistant markdown segments, after applying the same `MEDIA:` directive stripping OpenClaw ACP applies to the visible reply. Any ambiguity declines the repair: an unresolved turn anchor, a segment carrying more than one markdown part, or a differing count between transcript texts and streamed segments. Renderer-only compatibility segments are excluded from the pairing. A paired segment is rewritten only when the transcript projection differs from the streamed text and is not shorter, so a transcript read before the turn finished persisting cannot shorten a correct reply, and a whitespace-only difference never re-renders a settled bubble. It creates no turns, segments, tool cards, thoughts, or attachments.

**Reconciliation behavior.** The plan is computed and applied against the committed timeline inside a single state commit, is session- and generation-scoped, and is idempotent, so the retry attempt reports `already-matching` rather than repeating work. Every repair and every declined comparison is recorded on the ACP diagnostics trace as `assistant-text-repair:<status>`, which is what makes a future truncation attributable to a layer instead of guessed.

**Removal condition.** Retire this supplement once the ACP client consumes Gateway protocol v4 `deltaText`/`replace` operations with explicit sequencing under the matching `runId`, which makes a dropped chunk detectable and recoverable in band.
