...
- **💬 智能聊天界面**：多会话上下文与历史记录，流式 Markdown 渲染（语法高亮、CJK 排版、表格、KaTeX 公式）、`@agent` 直接路由与 `/技能` 内联卡片，内嵌子 Agent 状态、下钻及直接返回父会话，工作空间优先的会话侧边栏，以及 Markdown、`.docx`、`.pptx` 和本地 HTML 的只读预览。
- **🤖 Agent 生命周期管理**：可在桌面端创建和管理专用 Agent。删除非默认 Agent 时必须明确确认；此操作会永久删除其由 ClawX 管理的工作空间及所有关联聊天记录，相关会话和已删除的工作空间条目会立即从聊天界面消失且无法恢复。
- **🧰 导出问题现场**：前往“设置 > 支持”查看导出内容，选择一个或多个会话（支持全选），即可在桌面生成包含与底层存储无关的会话 JSONL 导出、已脱敏 OpenClaw 配置和可用诊断日志的 ZIP；完成后 ClawX 会显示保存路径。
...
- **配置交付**：Gateway 运行时由 Main 使用 `config.get` / `config.set`，停止或启动中则更新解析后的 JSON5 配置；普通 Provider/Agent/Skill/模型修改不会替换进程，有意删除 Agent 则使用 OpenClaw 专用的 `agents.delete` RPC。凭据通过 `secrets.reload` 热更新。QQBot 保存会写入稳定版插件要求的发送者白名单，启动清理也会修复旧版 ClawX 创建的配置且不会覆盖已有白名单。新建 Agent 是一个受控例外：如果凭据初始化生成了认证引导数据库，ClawX 会先完成一次受管理的迁移重启，再将 Agent 标记为可用。连续三分钟没有已验证的 Gateway 活动后，ClawX 会验证核心 RPC，并且只重启其自身拥有且不可用的 Gateway 进程；外部管理的 Gateway 保留给用户手动恢复。
- **OpenClaw 2026.8.2 存储与升级**：活动会话状态由 OpenClaw 基于 SQLite 的 Gateway 接口统一管理。历史、重命名、删除、用量统计、Cron 回放、频道目标和问题现场导出均通过会话 RPC 完成，不再直接修改正在使用的 `sessions.json` 或会话文件。凭据同步会保留 OpenClaw Agent 数据库的规范 schema 元数据；新建 Agent 的认证引导数据库会在首次使用前完成迁移与验证。执行受支持的原地升级时，ClawX 会先停止并发写入，在活动状态目录之外创建带校验和的恢复点，再于 Gateway 启动前运行 Doctor 的全 Agent SQLite 迁移与验证；任何阶段无法验证都会中止启动。恢复点会保留供显式恢复使用；若要降级，必须先恢复该备份，再启动旧版本运行时。
