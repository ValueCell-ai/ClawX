...
- **配置交付**：Gateway 运行时由 Main 使用 `config.get` / `config.set`，停止或启动中则更新解析后的 JSON5 配置；普通 Provider/Skill/模型修改不会替换进程，Agent 的创建和删除则使用 OpenClaw 专用的 `agents.create` 与 `agents.delete` RPC。删除受管理的 Agent 时由 OpenClaw 同时清理工作区文件及其引导证明状态，因此可安全重建同 ID Agent；必要时会认领并修复旧版已完成的删除记录，未完成的清理仍保持路径隔离。凭据通过 `secrets.reload` 热更新。QQBot 保存会写入稳定版插件要求的发送者白名单，启动清理也会修复旧版 ClawX 创建的配置且不会覆盖已有白名单。新 Agent 的凭据初始化会直接创建当前版本的 OpenClaw 数据库 schema，不再重启 Gateway；受管理的迁移重启仅作为已有旧数据库的兜底。连续三分钟没有已验证的 Gateway 活动后，ClawX 会验证核心 RPC，并且只重启其自身拥有且不可用的 Gateway 进程；外部管理的 Gateway 保留给用户手动恢复。
- **OpenClaw 2026.8.2 存储与升级**：活动会话状态由 OpenClaw 基于 SQLite 的 Gateway 接口统一管理。历史、重命名、删除、用量统计、Cron 回放、频道目标和问题现场导出均通过会话 RPC 完成，不再直接修改正在使用的 `sessions.json` 或会话文件。新 Agent 数据库由 OpenClaw 的规范初始化器直接创建，凭据同步会保留已有 schema 元数据，因此新 Agent 无需迁移重启即可使用当前 schema。执行受支持的原地升级时，ClawX 会先停止并发写入，在活动状态目录之外创建带校验和的恢复点，再于 Gateway 启动前运行 Doctor 的全 Agent SQLite 迁移与验证；任何阶段无法验证都会中止启动。恢复点会保留供显式恢复使用；若要降级，必须先恢复该备份，再启动旧版本运行时。
