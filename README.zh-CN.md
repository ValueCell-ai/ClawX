...
- **OpenClaw 2026.8.1 存储与升级**：活动会话状态由 OpenClaw 基于 SQLite 的 Gateway 接口统一管理。历史、重命名、删除、用量统计、Cron 回放、频道目标和问题现场导出均通过会话 RPC 完成，不再直接修改正在使用的 `sessions.json` 或会话文件。执行受支持的原地升级时，ClawX 会先停止并发写入，在活动状态目录之外创建带校验和的恢复点，再于 Gateway 启动前运行 Doctor 的全 Agent SQLite 迁移与验证；任何阶段无法验证都会中止启动。恢复点会保留供显式恢复使用；若要降级，必须先恢复该备份，再启动旧版本运行时。
