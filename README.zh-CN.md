...
- 在 **设置 → 高级 → 开发者** 中，Runtime Doctor 会在 OpenClaw 模式执行 `openclaw doctor --json`，在 cc-connect 模式执行内置的 `cc-connect doctor user-isolation`；Doctor Fix 仍只支持 OpenClaw。
