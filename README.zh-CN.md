...
开启**开发者模式**后，侧栏末尾会显示 Talk 入口，**设置 → Talk** 也会显示。Talk 会先检查目录就绪状态再启动。设置页列出 Gateway catalog 声明的全部实时提供商和模型，将未配置的提供商置灰，并通过 OpenClaw 配置事务保存所选提供商和模型。提供商凭据及语音音色等 provider-specific 选项仍由 Gateway 在 OpenClaw 配置文件中管理，设置页可直接打开该文件。
