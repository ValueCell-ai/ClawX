...
- **🔐 安全的供应商集成**：支持 OpenAI、Anthropic、Z.AI / GLM、TokenDance 等供应商，凭证经系统原生密钥链安全存储；TokenDance 支持带 PKCE 与 ClawX 请求归因的浏览器 OAuth，同时提供自定义 Provider 与兼容网关的降级探测。在开发者模式下，请前往 **模型 → 图像生成** 配置生图端点。
