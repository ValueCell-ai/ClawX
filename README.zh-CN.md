...
### 单页面 Web 浏览器
Chat 右侧面板通常包含四个选项卡：工作空间、预览、变更和网页浏览器。当预览正在显示 Excel 等非 HTML 文件时，网页浏览器选项卡会暂时隐藏，避免让之前的网页看起来与当前文件有关。网页浏览器会在首次使用时延迟创建一个实时页面；关闭面板、切换面板选项卡、切换聊天会话或前往 ClawX 的其它路由时，页面只会隐藏并继续运行，因此脚本、网络活动、音频和资源占用都可能持续。专用持久会话会在应用重启后保留 Cookie 和站点存储，但每次启动都从 `about:blank` 开始，不恢复上次的 URL、页面状态或导航历史。页面提供网站图标时，图标会显示在标题左侧；没有图标时，同尺寸占位图标会保持标题对齐，编辑地址时则隐藏整个图标位。该功能不提供额外浏览器标签页或窗口、书签、持久化历史、密码管理器或自动填充管理。

顶层导航支持 HTTP、HTTPS 和明确输入的标准 `file:///` URL；普通文件系统路径及其它协议会被拒绝。本地 `.html` 和 `.htm` 附件、文件变更及工作空间文件默认在 ClawX 网页浏览器中打开。Chat、Markdown 预览和执行详情中由 ClawX 渲染的 HTTP(S) 链接会显示为不可点击的普通文本。嵌入网页内部的 HTTP(S) 链接在点击时默认交给系统浏览器；右键链接可选择**在 ClawX 中打开**或**在系统浏览器中打开**，本地相对链接则继续留在 ClawX。打开本地文件会在 Chromium 的常规安全规则下向嵌入页面暴露其中可读取的内容；对 `file:` URL 使用**在系统浏览器中打开**时，操作系统也可能改用文件关联应用，而不是浏览器。弹窗不会创建子窗口：HTTP(S) 目标默认在外部打开，允许的本地文件目标可复用当前页面，但不会保留 `window.opener`、返回句柄或完整弹窗兼容性。
