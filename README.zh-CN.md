...
ClawX 会在 macOS 和 Windows 上由 Gateway 启动的 CLI 进程中关闭 CUA Driver 产品遥测，避免 Windows 上由遥测引发的终端闪窗。嵌入式 SDK 0.21.0 不允许传入该环境变量，因此不覆盖 daemon 的遥测设置。此设置不会修改系统环境变量、独立 CUA 安装的设置或 ClawX 自身的遥测偏好。
