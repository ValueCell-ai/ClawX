...
cc-connect 也负责消息平台桥接。当 cc-connect 是当前 runtime 时，频道状态探测会通过 runtime 抽象层路由，而不是继续固定查询 OpenClaw Gateway；已配置的频道账号会同步为 cc-connect project platform blocks，频道保存/删除会重启托管 cc-connect runtime，让 platform 变更立即生效；开发者模式侧边栏的页面入口会打开 cc-connect Web Admin。
