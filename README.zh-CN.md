...
- **🔐 安全的供应商集成**：支持 OpenAI、Anthropic、Z.AI / GLM 等供应商，凭证经系统原生密钥链安全存储；提供自定义 Provider、OAuth 登录、图像生成端点与兼容网关的降级探测。未设置请求超时的自定义 Provider 默认按每次尝试 45 秒处理，使无响应端点在触发整个 Gateway 恢复前先失败。
