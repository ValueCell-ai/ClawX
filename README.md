...
The bundled Computer Use plugin upgrades existing mirrors by version. After a plugin update, restart ClawX to load the new code. New driver proxy connections use separate sessions; failed desktop inputs are never automatically replayed.
...
The bundled `clawx-cua-computer` OpenClaw plugin registers the model-facing `computer` tool and uses a local MCP stdio proxy to reach the Main-owned driver. A separate first-party **computer-use** skill ships locally in both development and packaged builds. Select it in the chat skill picker to insert `/computer-use` for an explicit desktop task. Its English guidance covers screenshot-based actions, verification, and confirmation before destructive or external actions; it is not authorization or a safety guarantee, and selecting it does not enable Computer Use or grant OS permissions. Startup installs it into `~/.openclaw/skills/computer-use` without overwriting an existing same-name directory. No runtime skill download is needed.
