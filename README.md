...
- **📡 Multi-Channel Management**: Configure and monitor independent AI channels with multiple accounts, per-account agent binding, default-account switching, the bundled official Tencent personal WeChat channel plugin, and the official DingTalk connector remapped onto the existing `dingtalk` channel (calendar/docs need plugin skills plus `dws`).
