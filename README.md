...
- **Configuration delivery**: Main uses `config.get`/`config.set` while the Gateway is running and updates the resolved JSON5 config while it is stopped or starting; ordinary provider, agent, skill, and model changes do not replace the process, while intentional Agent deletion uses OpenClaw's dedicated `agents.delete` RPC. Credentials are hot-reloaded through `secrets.reload`. QQBot saves include the stable plugin's required sender allowlist, and startup sanitization repairs configurations created by older ClawX versions without replacing existing allowlists. Creating an Agent is the bounded exception: if credential setup creates an auth-bootstrap database, ClawX completes a managed migration restart before reporting the Agent ready. After three minutes without verified Gateway activity, ClawX verifies the core RPC and restarts only an unavailable Gateway process it owns; externally managed Gateways are left for manual recovery.
- **OpenClaw 2026.8.2 storage and upgrades**: Active session state is owned by OpenClaw's SQLite-backed Gateway contracts. ClawX uses session RPCs for history, rename, deletion, usage, cron replay, channel targets, and support exports instead of editing live `sessions.json` or transcript files. Credential synchronization preserves OpenClaw's canonical agent-database schema metadata; newly created auth-bootstrap databases are migrated and verified before first use. On a supported in-place upgrade, ClawX stops competing writers, creates a checksum-verified recovery checkpoint outside the active state directory, runs Doctor's all-agent SQLite migration and validation before Gateway launch, and fails closed if any stage cannot be proven. The checkpoint is retained for explicit recovery; downgrading requires restoring it before starting an older runtime.
