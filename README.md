...
- **🧰 Issue Report Export**: Open Settings > Support to review the bundle contents, select one or more conversations (or select all), and create a ZIP on the desktop with storage-neutral JSONL transcript exports, sanitized OpenClaw configuration, and available diagnostic logs; ClawX shows the saved path when complete.
...
- **Configuration delivery**: Main uses `config.get`/`config.set` while the Gateway is running and updates the resolved JSON5 config while it is stopped or starting; ordinary provider, skill, and model changes do not replace the process, while Agent creation and deletion use OpenClaw's dedicated `agents.create` and `agents.delete` RPCs. Managed Agent deletion lets OpenClaw remove both workspace files and their bootstrap attestation, so the same Agent ID can be recreated safely; completed legacy deletion journals are claimed and repaired when necessary, while unfinished cleanup remains fenced. Credentials are hot-reloaded through `secrets.reload`. QQBot saves include the stable plugin's required sender allowlist, and startup sanitization repairs configurations created by older ClawX versions without replacing existing allowlists. New Agent credential setup initializes the current OpenClaw database schema directly and does not restart the Gateway; the managed migration restart remains only for pre-existing outdated databases. After three minutes without verified Gateway activity, ClawX verifies the core RPC and restarts only an unavailable Gateway process it owns; externally managed Gateways are left for manual recovery.
- **OpenClaw 2026.8.2 storage and upgrades**: Active session state is owned by OpenClaw's SQLite-backed Gateway contracts. ClawX uses session RPCs for history, rename, deletion, usage, cron replay, channel targets, and support exports instead of editing live `sessions.json` or transcript files. Credential synchronization invokes OpenClaw's canonical initializer for new agent databases and preserves existing schema metadata, so new Agents start at the current schema without a migration restart. On a supported in-place upgrade, ClawX stops competing writers, creates a checksum-verified recovery checkpoint outside the active state directory, runs Doctor's all-agent SQLite migration and validation before Gateway launch, and fails closed if any stage cannot be proven. The checkpoint is retained for explicit recovery; downgrading requires restoring it before starting an older runtime.
