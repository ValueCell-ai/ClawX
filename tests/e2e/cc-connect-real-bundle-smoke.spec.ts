...
async function createDeterministicCodexAppServer(root: string): Promise<string> {
  const binDir = join(root, 'bin');
  const scriptPath = join(binDir, 'mock-app-server.cjs');
  const binaryPath = join(binDir, process.platform === 'win32' ? 'codex.cmd' : 'codex');
  await mkdir(binDir, { recursive: true });
  await writeFile(scriptPath, `const readline = require('node:readline');
const args = process.argv.slice(2);
if (args.includes('--version')) {
  process.stdout.write('codex-cli 0.137.0 deterministic-app-server\\n');
  process.exit(0);
}
if (args[0] !== 'app-server') {
  process.stderr.write('expected app-server, got ' + JSON.stringify(args));
  process.exit(2);
}
...
rl.on('line', (line) => {
  ...
  if (request.method === 'initialize') {
    send({ jsonrpc: '2.0', id: request.id, result: { protocolVersion: '2026-06-01' } });
    return;
  }
  if (request.method === 'thread/start') {
    send({ jsonrpc: '2.0', id: request.id, result: {
      cwd: process.cwd(), model: 'gpt-5.4-mini', reasoningEffort: 'medium', thread: { id: 'thread-rich-preview' },
    } });
    return;
  }
  if (request.method === 'account/rateLimits/read') {
    send({ jsonrpc: '2.0', id: request.id, result: { rateLimits: {}, rateLimitsByLimitId: {} } });
    return;
  }
  if (request.method === 'turn/start') {
    const turnId = 'turn-rich-preview';
    send({ jsonrpc: '2.0', id: request.id, result: { turn: { id: turnId } } });
    notify('turn/started', { threadId: 'thread-rich-preview', turn: { id: turnId, status: 'inProgress' } });
    setTimeout(() => notify('item/completed', {
      threadId: 'thread-rich-preview', turnId,
      item: { id: 'reasoning-1', type: 'reasoning', summary: ['Inspecting through cc-connect'] },
    }), 50);
    setTimeout(() => notify('item/started', {
      threadId: 'thread-rich-preview', turnId,
      item: { id: 'command-1', type: 'commandExecution', command: 'printf cc-connect-preview', status: 'inProgress' },
    }), 1_200);
    setTimeout(() => notify('item/completed', {
      threadId: 'thread-rich-preview', turnId,
      item: {
        id: 'command-1', type: 'commandExecution', command: 'printf cc-connect-preview',
        status: 'completed', aggregatedOutput: 'cc-connect-preview', exitCode: 0,
      },
    }), 2_400);
    setTimeout(() => notify('item/completed', {
      threadId: 'thread-rich-preview', turnId,
      item: { id: 'message-1', type: 'agentMessage', text: 'CLAWX_REAL_RICH_PREVIEW_OK' },
    }), 2_700);
    setTimeout(() => notify('turn/completed', {
      threadId: 'thread-rich-preview', turn: { id: turnId, status: 'completed' },
    }), 2_850);
    return;
  }
  send({ jsonrpc: '2.0', id: request.id, result: {} });
...
test.describe('cc-connect real runtime bundle smoke', () => {
  ...
  test('renders progress generated by the real cc-connect engine through preview and update packets', async ({
    launchElectronApp,
    userDataDir,
  }) => {
    test.setTimeout(180_000);
    const bundles = await realRuntimeBundles();
    test.skip(!bundles, 'Run pnpm run bundle:cc-connect:current first.');
    await waitForPortClosed(9810);
    await waitForPortClosed(9820);
    await seedCcConnectRuntimeSettings(userDataDir);
    const codexPath = await createDeterministicCodexAppServer(join(userDataDir, 'deterministic-codex'));
    ...
    try {
      ...
      await page.evaluate(() => {
        const testWindow = window as typeof window & {
          __ccConnectRichRuntimeEvents?: Array<Record<string, unknown>>;
        };
        testWindow.__ccConnectRichRuntimeEvents = [];
        window.electron.ipcRenderer.on('chat:runtime-event', (payload) => {
          testWindow.__ccConnectRichRuntimeEvents!.push(payload as Record<string, unknown>);
        });
      });

      await expect.poll(async () => await page.evaluate(async () => window.clawx.hostInvoke({
        id: 'runtime-start-real-rich-progress',
        module: 'gateway',
        action: 'start',
      ...
      await page.getByTestId('chat-composer-input').fill('Exercise the cc-connect progress bridge.');
      await page.getByTestId('chat-composer-send').click();
      await expect(page.getByTestId('chat-message-role-assistant').filter({ hasText: 'CLAWX_REAL_RICH_PREVIEW_OK' }).last())
      ...
      const events = await page.evaluate(() => {
        ...
        return testWindow.__ccConnectRichRuntimeEvents ?? [];
      });
      ...
      const diagnostics = await page.evaluate(async () => window.clawx.hostInvoke({
        id: 'runtime-diagnostics-real-rich-progress',
        module: 'diagnostics',
        action: 'gatewaySnapshot',
      })) as {
        ok: boolean;
        data?: { runtime?: { ccConnect?: { logTail?: string } } };
      };
      expect(diagnostics).toMatchObject({ ok: true });
      const logTail = diagnostics.data?.runtime?.ccConnect?.logTail;
      expect(logTail).toContain('progress packet type=preview_start');
      ...
      await page.screenshot({
        path: join(evidenceDir, 'real-rich-progress-bridge.png'),
        fullPage: true,
      });
      await writeFile(join(evidenceDir, 'real-rich-progress-bridge.json'), `${JSON.stringify({
        schema: 'clawx-cc-connect-real-rich-progress-evidence',
        version: 1,
        runtimeKind: 'cc-connect',
        runtimeBinary: 'real-bundled-v1.4.1',
        codexBoundary: 'deterministic-app-server-protocol-double',
        publicBridgeProtocol: true,
        packetTypesObserved: ['preview_start', 'update_message'],
        runtimeEventTypes: Array.from(new Set(eventTypes)),
