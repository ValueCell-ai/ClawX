...
export function createFilesApi(dependencies: FilesApiDependencies = {}): CompleteHostServiceRegistry['files'] {
  ...
  const getOutboundDir = () => getRuntimeOutboundMediaDir(dependencies.runtimeManager);
  return {
    ...
    readText: async (payload) => {
      try {
        const { realPath: real, readOnly } = await resolveSandboxedPath(requirePath(payload), 'read', dependencies.runtimeManager);
        const fsP = await import('node:fs/promises');
    ...
    writeText: async (payload) => {
      try {
        ...
        const { realPath: real } = await resolveSandboxedPath(requirePath(payload), 'write', dependencies.runtimeManager);
        const fsP = await import('node:fs/promises');
        let stat;
        try {
    ...
    listDir: async (payload) => {
      try {
        const { realPath: real } = await resolveSandboxedPath(requirePath(payload), 'read', dependencies.runtimeManager);
        const fsP = await import('node:fs/promises');
