...
- **📡 マルチチャネル管理**：複数アカウント、アカウント単位のAgent紐付け、既定アカウントの切り替え、Tencent公式個人WeChatチャネルプラグイン、既存の `dingtalk` チャネルにリマップした公式 DingTalk コネクタを備えています。DingTalk 追加後、カレンダーやドキュメントなどの `dws` スキル向けに任意の DingTalk Workspace OAuth を案内し、スキップしてもチャットには影響しません。組織の主管理者は、事前に DingTalk 開発者コンソールでメンバーによる CLI からの個人データアクセスを有効にする必要があります。
