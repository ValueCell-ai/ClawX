...
ACP Chat は、runtime が画像生成メディアを信頼できる構造化メディアとして配信した場合に、生成画像のプレビューも表示できます。信頼できる OpenClaw internal-UI 配信と画像生成タスクに関連付けられた最終返信では、テキストのみの失敗説明を含む元のユーザー向け完了テキストを保持し、汎用の画像キャプションへ置き換えません。履歴 ACP の読み込みには明示的な OpenClaw session ルーティングメタデータを付与し、新しい metadata-only replay ledger が正規 transcript を隠さないようにします。また起動時は Gateway ready 後のセッション検出を待ってから、既定セッションが存在しないかを判断します。画像完了タスクの内部ユーザートリガーは timeline に表示しません。OpenClaw の履歴リプレイ中は、画像タスクが ACP の task/tool アンカーと一致する場合、元の transcript ユーザーターンが読み込まれた ACP ユーザーターンと正確に対応する場合、または最初に対応した ACP リプレイターンの直前にある単一の画像ターンである場合に限り、assistant の画像 `MEDIA:` マーカーがインライン画像表示へ昇格されます。この限定的な境界画像はリプレイ範囲の前に挿入され、新しいターンの末尾には追加されません。ClawX は Renderer から任意にファイルシステムへアクセスするのではなく、Electron Main のホストメディア処理を通じてプレビューを読み込みます。標準 ACP の画像と resource コンテンツは引き続き推奨パスであり、そのまま描画されます。
