...
- **OpenClaw 2026.8.1 のストレージとアップグレード**：アクティブなセッション状態は、OpenClaw の SQLite ベースの Gateway 契約が管理します。履歴、名前変更、削除、使用量、Cron 再生、チャネルターゲット、サポートエクスポートにはセッション RPC を使用し、稼働中の `sessions.json` やトランスクリプトを直接変更しません。対応するインプレースアップグレードでは、競合する書き込みを停止し、アクティブな状態ディレクトリ外にチェックサム検証済みの復旧ポイントを作成してから、Gateway 起動前に Doctor の全 Agent SQLite 移行と検証を実行します。証明できない段階があれば起動を中止します。復旧ポイントは明示的な復元のため保持され、ダウングレード時は旧ランタイムの起動前に復元が必要です。
