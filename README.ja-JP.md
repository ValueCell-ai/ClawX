...
ClawXにはベストプラクティスに基づくモデルプロバイダーがあらかじめ設定されており、Windowsと多言語設定をネイティブにサポートしています。コンパクション予約は選択したモデルのコンテキストウィンドウに自動追従し、完了済みターンは圧縮後に逐語的に再生されず、要約を通じて引き継がれます。開発者モードでは適用された予約値を確認できます。高度な設定は **設定 → 詳細設定 → 開発者モード** から調整できます。
...
- **💬 インテリジェントチャットインターフェース**：複数セッションのコンテキストと履歴、シンタックスハイライト付きストリーミングMarkdown、CJK対応解析、テーブル、KaTeX数式、`@agent` による直接ルーティング、インライン `/skill` カード、ワークスペース優先のセッション、Markdown・`.docx`・`.pptx`・ローカルHTMLの読み取り専用プレビューに対応します。完了した返信は永続化transcriptと照合されるため、ストリーミングのチャンクが失われても画面に不完全な回答が残りません。
- **🤖 Agentライフサイクル管理**：デスクトップから専用Agentを作成・管理できます。既定以外のAgentを削除するには明示的な確認が必要です。削除するとClawX管理下のワークスペースと関連するすべてのチャット履歴が完全に消去され、該当する会話はサイドバーから直ちに消え、復元できません。
