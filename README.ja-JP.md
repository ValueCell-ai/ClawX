...
- **💬 インテリジェントチャットインターフェース**：複数セッションのコンテキストと履歴、シンタックスハイライト付きストリーミングMarkdown、CJK対応解析、テーブル、KaTeX数式、`@agent` による直接ルーティング、インライン `/skill` カード、埋め込みサブエージェントの状態表示・子会話への移動・直接の親会話への復帰、ワークスペース優先のセッション、Markdown・`.docx`・`.pptx`・ローカルHTMLの読み取り専用プレビューに対応します。
...
- **🧰 問題調査レポートのエクスポート**：「設定 > サポート」で内容を確認し、1つ以上の会話（すべて選択も可能）を選ぶと、ストレージ形式に依存しない会話の JSONL エクスポート、認証情報を除去した OpenClaw 設定、利用可能な診断ログを含む ZIP をデスクトップに作成し、完了後に保存先を表示します。
...
- **設定の配信**：Gateway実行中は `config.get` / `config.set` を使い、停止中または起動中は解決済みJSON5設定を更新します。通常のプロバイダー、Agent、スキル、モデル変更ではプロセスを置き換えず、意図的なAgent削除にはOpenClaw専用の `agents.delete` RPCを使用します。認証情報は `secrets.reload` でホットリロードされます。QQBotの保存時には安定版プラグインが必須とする送信者許可リストを追加し、起動時のサニタイズでは既存の許可リストを保持したまま旧版ClawXの設定を修復します。Agent作成は限定的な例外です。認証情報の初期化で認証ブートストラップデータベースが作成された場合、ClawXはAgentを利用可能として返す前に管理対象の移行再起動を完了します。検証済みのGatewayアクティビティが3分間ない場合、ClawXはコアRPCを検証し、自身が所有する利用不能なGatewayプロセスだけを再起動します。外部管理のGatewayは手動で復旧します。
- **OpenClaw 2026.8.2 のストレージとアップグレード**：アクティブなセッション状態は、OpenClaw の SQLite ベースの Gateway 契約が管理します。履歴、名前変更、削除、使用量、Cron 再生、チャネルターゲット、サポートエクスポートにはセッション RPC を使用し、稼働中の `sessions.json` やトランスクリプトを直接変更しません。認証情報の同期ではOpenClaw Agentデータベースの正規schemaメタデータを保持し、新規Agentの認証ブートストラップデータベースは初回利用前に移行・検証されます。対応するインプレースアップグレードでは、競合する書き込みを停止し、アクティブな状態ディレクトリ外にチェックサム検証済みの復旧ポイントを作成してから、Gateway 起動前に Doctor の全 Agent SQLite 移行と検証を実行します。証明できない段階があれば起動を中止します。復旧ポイントは明示的な復元のため保持され、ダウングレード時は旧ランタイムの起動前に復元が必要です。
