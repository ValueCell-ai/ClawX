...
- **🔐 セキュアなプロバイダー統合**：OpenAI、Anthropic、Z.AI / GLM、TokenDanceなどに接続し、認証情報をOSのネイティブキーチェーンに安全に保存できます。TokenDanceではPKCEとClawXリクエスト帰属付きのブラウザOAuthに対応し、カスタムプロバイダーや互換性フォールバックも利用できます。開発者モードでは **Models → Image Generation** で画像生成エンドポイントを設定できます。
