...
内蔵の `clawx-cua-computer` は OpenClaw プラグインで、モデル向けの `computer` ツールを登録し、ローカル MCP stdio プロキシで Main 管理のドライバーに接続します。Skill ではなく、Skill の追加インストールも行いません。操作の説明はツールの説明とパラメーター定義に含まれます。
