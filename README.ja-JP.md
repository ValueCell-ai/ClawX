...
ClawX は macOS と Windows の Gateway 経由の CLI プロセスで CUA Driver の製品テレメトリを無効にし、Windows でテレメトリに起因するコンソールの点滅を回避します。組み込み SDK 0.21.0 はこの環境変数の上書きを拒否するため、daemon のテレメトリ設定は上書きしません。システム環境変数、単独の CUA インストールの設定、ClawX 自体のテレメトリ設定は変更しません。
...
同梱の **computer-use** Skill は `/computer-use` コマンドを維持し、[CUA 0.21.0 に付属する公式 Skill](https://github.com/trycua/cua/tree/70db98d1bcd92890d778f4978e0eb107a4b66c1b/libs/cua-driver/rust/Skills/cua-driver)に基づいています。参照元はタグ `cua-driver-rs-v0.21.0` に固定され、更新され続ける上流の `main` ではありません。MIT ライセンスの文書とライセンスをオフラインで同梱し、Main の接続先、権限、セッション、画像処理に関する短い ClawX 専用ガイドを入口にしています。起動時に `~/.openclaw/skills/computer-use` へインストールし、既知の旧同梱ガイドを置き換えますが、無関係なユーザーコンテンツは保持します。Skill の選択でサービスや OS 権限は有効になりません。[Skill の出典と統合](harness/reference/computer-use-skill.md)を参照してください。
