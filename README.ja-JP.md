...
- **設定の配信**：Gateway実行中は `config.get` / `config.set` を使い、停止中または起動中は解決済みJSON5設定を更新します。通常のプロバイダー、スキル、モデル変更ではプロセスを置き換えず、Agentの作成と削除にはOpenClaw専用の `agents.create` と `agents.delete` RPCを使用します。管理対象Agentの削除ではOpenClawがワークスペースのファイルとブートストラップ証明状態をまとめて消去するため、同じIDのAgentを安全に再作成できます。必要に応じて旧版の完了済み削除記録も引き継いで修復し、未完了のクリーンアップは引き続きパスを保護します。認証情報は `secrets.reload` でホットリロードされます。QQBotの保存時には安定版プラグインが必須とする送信者許可リストを追加し、起動時のサニタイズでは既存の許可リストを保持したまま旧版ClawXの設定を修復します。新規Agentの認証情報設定では現在のOpenClawデータベースschemaを直接初期化するため、Gatewayを再起動しません。管理対象の移行再起動は既存の古いデータベースに対するフォールバックに限定されます。検証済みのGatewayアクティビティが3分間ない場合、ClawXはコアRPCを検証し、自身が所有する利用不能なGatewayプロセスだけを再起動します。外部管理のGatewayは手動で復旧します。
- **OpenClaw 2026.8.2 のストレージとアップグレード**：アクティブなセッション状態は、OpenClaw の SQLite ベースの Gateway 契約が管理します。履歴、名前変更、削除、使用量、Cron 再生、チャネルターゲット、サポートエクスポートにはセッション RPC を使用し、稼働中の `sessions.json` やトランスクリプトを直接変更しません。新規AgentデータベースはOpenClawの正規初期化処理で直接作成され、認証情報の同期では既存のschemaメタデータを保持するため、移行再起動なしで現在のschemaを利用できます。対応するインプレースアップグレードでは、競合する書き込みを停止し、アクティブな状態ディレクトリ外にチェックサム検証済みの復旧ポイントを作成してから、Gateway 起動前に Doctor の全 Agent SQLite 移行と検証を実行します。証明できない段階があれば起動を中止します。復旧ポイントは明示的な復元のため保持され、ダウングレード時は旧ランタイムの起動前に復元が必要です。
