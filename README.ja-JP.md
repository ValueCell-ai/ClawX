...
- **💻 ローカルComputer Use**：macOS 13以上（IntelまたはAppleシリコン）とWindows x64では、ClawXに同梱されたドライバーを使ってAgentがプライマリディスプレイを取得・操作できます。この機能はローカル環境に限定され、OpenClawノードのペアリングや実行時の追加ダウンロードは不要です。
...
- **Computer Use**：macOS 13以上のx64/arm64、またはWindows 10以上のx64。他の対応プラットフォームでもClawXは利用できますが、このツールは提供されません
- **メモリ**：最低4GB RAM（8GB推奨）
- **ストレージ**：1GBの空きディスク容量
...
Computer Use は任意の機能で、明示的な設定のない既存環境も含めて**初期状態では無効**です。サイドバーの**コンピューター操作**で有効にできます。対応する macOS と Windows では、内蔵ドライバーが `computer` ツールでプライマリ画面の取得、移動、クリック、ドラッグ、スクロール、入力、キー操作、上限付き待機を提供します。設定は再起動後も保持されます。無効にするとドライバーを停止し、接続情報を削除して Agent 向け Gateway プラグインを無効にします。設定の再読み込みで現在の作業が中断される場合があります。リモート検出や OpenClaw ノードのペアリングは行いません。

macOS の管理画面では**アクセシビリティ**と**画面収録**の状態を読み取り専用で表示します。起動、アクティブ化、トグルの有効化では権限を要求しません。有効にしてから**権限を要求**を押してください。拒否した場合はシステム設定 > プライバシーとセキュリティで ClawX を許可します。変更後に再起動が必要な場合があります。機能を無効にしても OS の許可は取り消されません。権限やドライバーが不足しても Chat や Gateway の起動は妨げません。

内蔵の `clawx-cua-computer` は OpenClaw プラグインで、モデル向けの `computer` ツールを登録し、ローカル MCP stdio プロキシで Main 管理のドライバーに接続します。開発環境と配布版には、独立したファーストパーティーの **computer-use** Skill もローカルに同梱されます。チャットのスキル選択から `/computer-use` を挿入し、明示的なデスクトップ操作タスクに使用できます。英語のガイドはスクリーンショットに基づく操作、結果確認、破壊的操作や外部への操作前の確認を扱いますが、権限付与や安全性の保証ではありません。Skill の選択で Computer Use や OS 権限が有効になることはありません。起動時に `~/.openclaw/skills/computer-use` へインストールされ、既存の同名ディレクトリは上書きされません。実行時の Skill ダウンロードも不要です。
...
- **ローカルComputer Use**：Electron Mainが権限を持つ同梱CUA daemonとmacOS権限確認を直接管理します。Gatewayには非公開でgeneration単位のMCP起動descriptorだけを渡し、同梱プラグインは権限を持たないローカルproxyだけを起動します。node host、ペアリング、リモート検出、実行時ダウンロードは使用しません。
