...
トップレベルナビゲーションでは HTTP、HTTPS、および明示的に入力した標準 `file:///` URL を利用できます。通常のファイルシステムパスとその他のプロトコルは拒否されます。ローカルの `.html` / `.htm` 添付ファイル、ファイル変更、ワークスペースファイルは既定で ClawX のウェブブラウザに開きます。HTTP(S) リンクはクリックするとシステムブラウザで開き、右クリックすると **ClawX で開く**または**システムブラウザで開く**を選べます。ローカルの相対リンクは ClawX 内に留まります。ローカルファイルを開くと、通常の Chromium セキュリティ規則の範囲で、読み取り可能な内容が埋め込みページに公開されます。また、`file:` URL に **システムブラウザで開く**を使うと、OS の関連付け済みアプリが起動する場合があります。ポップアップは子ウィンドウを作らず、HTTP(S) は外部で開き、許可されたローカルファイルのみ現在のページを再利用できますが、`window.opener`、返されたハンドル、完全なポップアップ互換性は維持されません。
