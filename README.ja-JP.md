...
対応するmacOSおよびWindows環境では、ClawXがアプリに同梱されたComputer Useドライバーをローカルで起動し、Agentに `computer` ツールを提供します。プライマリディスプレイの取得、ポインター移動、クリック、ドラッグ、スクロール、テキスト入力、キー操作、上限付き待機を実行できます。リモートマシンの検出・操作やOpenClawノードのペアリングは行いません。

macOSでは、求められたときにClawXへ **アクセシビリティ** と **画面収録** の両方を許可してください。拒否後にシステム設定で許可した場合、またはClawXの実行中に許可を取り消した場合は、ClawXへ戻って再起動してから再試行してください。権限または同梱ドライバーが不足している場合、このツールだけが利用不能となり、ChatやGatewayの起動は妨げません。
