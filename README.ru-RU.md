...
### Опциональная автоматизация X/Twitter с TweetClaw
Команды, которым нужны OpenClaw-агенты для публичного социального исследования или аккаунтных X/Twitter workflows, могут установить отдельный плагин [TweetClaw](https://github.com/Xquik-dev/tweetclaw) в среду выполнения, управляемую ClawX. [Руководство по workflow](docs/tweetclaw-x-twitter-workflow.md) описывает установку, работу с учётными данными, allow-list для инструментов `explore`/`tweetclaw`, проверку в ClawX, рецепты Cron и проверки перед post tweets, post tweet replies, direct messages, media uploads, webhooks и giveaway draws.
