...
- **Доставка конфигурации**: изменения среды выполнения используют авторитетный снимок `config.set`, поэтому обычные изменения провайдера, агента, навыка и модели не заменяют процесс Gateway; для намеренного удаления агента используется специальный RPC OpenClaw `agents.delete`. Учётные данные обновляются без перезапуска через `secrets.reload`. При сохранении QQBot добавляется обязательный для стабильной версии плагина список разрешённых отправителей, а очистка при запуске исправляет конфигурации старых версий ClawX, не заменяя существующие списки. Создание агента является ограниченным исключением: если инициализация учётных данных создаёт загрузочную базу аутентификации, ClawX завершает управляемый перезапуск с миграцией, прежде чем пометить агента готовым.
- **Хранилище и обновление OpenClaw 2026.8.2**: активным состоянием сессий управляют SQLite-контракты Gateway OpenClaw. ClawX использует RPC сессий для истории, переименования, удаления, статистики, повторов Cron, целей каналов и экспорта отчётов, не изменяя активные `sessions.json` или файлы транскриптов напрямую. Синхронизация учётных данных сохраняет канонические метаданные схемы базы агента OpenClaw; загрузочная база нового агента мигрируется и проверяется до первого использования. При поддерживаемом обновлении на месте ClawX останавливает конкурирующие записи, создаёт проверенную контрольными суммами точку восстановления вне активного каталога состояния и до запуска Gateway выполняет миграцию и проверку SQLite для всех агентов через Doctor. Если этап не подтверждён, запуск блокируется. Точка восстановления сохраняется; для отката её необходимо восстановить до запуска старой среды выполнения.
