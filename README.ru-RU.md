...
- **🎙️ Голосовой ввод**: Диктуйте в поле чата — кнопка микрофона записывает голос, распознаёт речь через настроенный OpenAI-совместимый сервис и вставляет текст в позицию курсора. Настройка выполняется на вкладке **Модели → Распознавание речи**: выберите пресет (OpenAI, Groq, SiliconFlow или свой) и укажите API-ключ.
