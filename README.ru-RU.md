...
- **Доставка конфигурации**: изменения среды выполнения используют авторитетный снимок `config.set`, поэтому обычные изменения провайдера, навыка и модели не заменяют процесс Gateway; для создания и удаления агентов используются специальные RPC OpenClaw `agents.create` и `agents.delete`. При удалении управляемого агента OpenClaw одновременно удаляет файлы рабочего пространства и состояние их аттестации, поэтому агент с тем же ID можно безопасно создать снова; при необходимости завершённые записи удаления старых версий снимаются и исправляются, а незавершённая очистка продолжает защищать свои пути. Учётные данные обновляются без перезапуска через `secrets.reload`. При сохранении QQBot добавляется обязательный для стабильной версии плагина список разрешённых отправителей, а очистка при запуске исправляет конфигурации старых версий ClawX, не заменяя существующие списки. При настройке учётных данных нового агента сразу инициализируется текущая схема базы OpenClaw без перезапуска Gateway; управляемый перезапуск с миграцией остаётся только резервным путём для существующих устаревших баз.
- **Хранилище и обновление OpenClaw 2026.8.2**: активным состоянием сессий управляют SQLite-контракты Gateway OpenClaw. ClawX использует RPC сессий для истории, переименования, удаления, статистики, повторов Cron, целей каналов и экспорта отчётов, не изменяя активные `sessions.json` или файлы транскриптов напрямую. Новые базы агентов создаются каноническим инициализатором OpenClaw, а синхронизация учётных данных сохраняет существующие метаданные схемы, поэтому новая база сразу использует текущую схему без перезапуска для миграции. При поддерживаемом обновлении на месте ClawX останавливает конкурирующие записи, создаёт проверенную контрольными суммами точку восстановления вне активного каталога состояния и до запуска Gateway выполняет миграцию и проверку SQLite для всех агентов через Doctor. Если этап не подтверждён, запуск блокируется. Точка восстановления сохраняется; для отката её необходимо восстановить до запуска старой среды выполнения.
