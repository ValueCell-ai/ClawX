...
- **Доставка конфигурации**: изменения среды выполнения используют авторитетный снимок `config.set`, поэтому обычные изменения провайдера, агента, навыка и модели не заменяют процесс Gateway; учётные данные обновляются без перезапуска через `secrets.reload`. Создание агента является ограниченным исключением: если инициализация учётных данных создаёт загрузочную базу аутентификации, ClawX завершает управляемый перезапуск с миграцией, прежде чем пометить агента готовым.
