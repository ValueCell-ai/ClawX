...
- **Доставка конфигурации**: изменения среды выполнения используют авторитетный снимок `config.set`, поэтому обычные изменения провайдера, агента, навыка и модели не заменяют процесс Gateway; учётные данные обновляются без перезапуска через `secrets.reload`. При сохранении QQBot добавляется обязательный для стабильной версии плагина список разрешённых отправителей, а очистка при запуске исправляет конфигурации старых версий ClawX, не заменяя существующие списки. Создание агента является ограниченным исключением: если инициализация учётных данных создаёт загрузочную базу аутентификации, ClawX завершает управляемый перезапуск с миграцией, прежде чем пометить агента готовым.
