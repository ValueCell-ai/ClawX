...
- **🎙️ Голосовой ввод (режим разработчика)**: После включения **режима разработчика** в настройках можно диктовать в поле чата — кнопка микрофона записывает голос, распознаёт речь через настроенный OpenAI-совместимый сервис и вставляет текст в позицию курсора. Настройка выполняется на вкладке **Модели → Распознавание речи**: выберите пресет (OpenAI, Groq, SiliconFlow или свой) и укажите API-ключ.
  - При нажатии на микрофон проверяется разрешение. Если доступ запрещён, диалог предлагает явно открыть системные настройки; ограничения политики могут потребовать помощи администратора. На macOS разрешите ClawX в разделе «Конфиденциальность и безопасность → Микрофон», затем полностью закройте и перезапустите приложение. При разработке разрешение может относиться к IDE или терминалу запуска — их тоже нужно перезапустить. На Windows разрешите классическим приложениям доступ к микрофону. Закройте диалог и снова нажмите на микрофон для повторной попытки; запись не начинается автоматически.
