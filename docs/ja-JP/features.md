...
ACP の順序付きプロセスタイムラインには、コンテキストのコンパクション状態も表示されます。ライブマーカーは同じ位置で、コンパクション中から完了、コンパクション後に継続、失敗、またはキャンセルへ更新されます。履歴の完了マーカーは、OpenClaw の上限付き ACP 履歴レスポンスに含まれるコンパクションレコードについて順番どおりに復元されます。このビューはレスポンス範囲外の古いレコードを読み込まず、マーカーにコンパクション要約を表示することもありません。
