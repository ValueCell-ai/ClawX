...
リクエストタイムアウトが未設定のCustomプロバイダーには `timeoutSeconds = 45` が自動設定され、旧バージョンで保存されたエントリも起動時に修復されます。これにより、応答しないOpenAI互換リクエストはGateway全体の生存性回復が始まる前に失敗します。明示的に設定された `timeoutSeconds` は変更されません。
