...
アクティブな OpenClaw ACP セッションがコンテキスト使用量を報告すると、コンポーザーにリングとローカライズされた使用率を備えたコンパクトなメーターが表示されます。メーターにホバーまたはフォーカスすると、使用済み/合計トークン数を確認できます。使用済み/合計トークン数は通常、アクティブな ACP の `usage_update` から取得します。モデル切り替え後は、ACP が新しい使用量を報告するまで、更新された Agent スナップショットの有効なコンテキストウィンドウが古い合計値を置き換えるため、上限と使用率がすぐに更新されます。使用量メタデータがない場合や無効な場合は表示されません。
