...
The ordered ACP process timeline also shows context compaction. A live marker moves in place from compacting to completed, continuing, failed, or cancelled, while historical completed markers are restored in order for compaction records included in OpenClaw's bounded ACP history response. Older records outside that response are not loaded by this view, and markers never display the compaction summary.
