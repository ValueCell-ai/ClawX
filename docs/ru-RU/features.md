...
ClawX сохраняет явно заданные `contextWindow` или `contextTokens` в строках моделей провайдера, но не определяет отсутствующий предел контекста по имени модели. Резерв сжатия равен 25% только при наличии явного действительного предела; иначе `reserveTokensFloor` получает консервативное значение 50000 токенов. Если настройки сжатия отсутствуют, ClawX также создаёт `agents.defaults.compaction.mode = "safeguard"`, `keepRecentTokens = 0`, `recentTurnsPreserve = 0` и `midTurnPrecheck.enabled = true`. Синхронизация при запуске всегда устанавливает оба параметра сохранения истории в `0`, чтобы все завершённые ходы попадали в сводку, а не воспроизводились дословно после сжатия; явный выбор `midTurnPrecheck.enabled` сохраняется.

Z.AI (CN / Global) соответствует встроенному провайдеру OpenClaw `zai` (`ZAI_API_KEY`). Модель по умолчанию — `glm-5.3-flash`: она принимает изображения и сохраняет контекст в 1 млн токенов, тогда как `glm-5.3` с тем же контекстом работает только с текстом. Пресет Code Plan использует эндпоинты Coding Plan (`.../api/coding/paas/v4`), а обычные API — (`.../api/paas/v4`). CN и Global взаимоисключающие, поскольку используют один ключ среды выполнения OpenClaw.

DeepSeek соответствует встроенному провайдеру OpenClaw `deepseek` (`DEEPSEEK_API_KEY`). Модель по умолчанию — `deepseek-flash` (DeepSeek-V4.1-Flash); она регистрируется как поддерживающая изображения, поэтому в чат DeepSeek можно отправлять снимки экрана и картинки. Снятые с поддержки идентификаторы `deepseek-v4-flash` и `deepseek-v4-flash-vision-exp` обрабатываются так же, потому что DeepSeek по-прежнему направляет их в V4.1-Flash, а `deepseek-v4-pro` остаётся текстовым.

Остальные встроенные модели по умолчанию тоже соответствуют текущему поколению с контекстом в 1 млн токенов и принимают изображения: Anthropic — `claude-opus-5`, Google — `gemini-3.8-flash`, Moonshot (CN / Global) — `kimi-k3`, OpenRouter — `~deepseek/deepseek-flash-latest`. Префикс `~` в OpenRouter обозначает плавающий алиас «latest», закреплённый аналог — `deepseek/deepseek-v4.1-flash`. У SiliconFlow по умолчанию используется `zai-org/GLM-5.3`: тот же контекст в 1 млн токенов, но только текст; если нужны изображения, выберите там `zai-org/GLM-5.3-Flash`.

Anthropic и Google теперь явно регистрируются в конфигурации среды выполнения OpenClaw — так же, как все остальные встроенные провайдеры. Поэтому при сохранении такой учётной записи её ключ и выбранная модель применяются сразу, а идентификатор модели новее собственного каталога встроенной среды выполнения всё равно разрешается.

Если совместимый шлюз отклоняет `/models` по причинам, не связанным с аутентификацией, ClawX во время проверки API-ключа автоматически переключается на лёгкий зонд `/chat/completions` или `/responses` с настроенной моделью. Для ключей Anthropic и Google проверка дополнительно сравнивает настроенную модель со списком моделей поставщика и прямо называет недоступную модель, вместо того чтобы сохранить провайдера, который откажет на первом же запросе. Совместимые ретрансляторы из этого сравнения исключены: их списки могут раскрывать лишь часть фактически доступных моделей.
