...
Для Custom-провайдеров без тайм-аута запроса автоматически задаётся `timeoutSeconds = 45`; записи, сохранённые старыми версиями, также исправляются при запуске. Благодаря этому зависший OpenAI-совместимый запрос завершается до восстановления работоспособности всего Gateway. Явное значение `timeoutSeconds` остаётся без изменений.
