...
当活动的 OpenClaw ACP 会话上报上下文用量时，输入框会显示一个带用量环和本地化百分比的紧凑进度指示器。悬停或聚焦该指示器可查看已用/总 token 数。已用和总 token 通常来自活动 ACP 的 `usage_update`；切换模型后，在 ACP 上报新用量前，更新后的 Agent 快照中的有效上下文窗口会替换旧总量，因此上限和百分比会立即更新；用量元数据缺失或无效时不会显示。
